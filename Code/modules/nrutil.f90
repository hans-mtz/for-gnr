MODULE NRUTIL
   use iso_c_binding, only: c_float, c_int, c_double
...
CONTAINS
  ...
  pure REAL(c_double) FUNCTION DOTP(a,b)
    IMPLICIT NONE
  ...
  pure FUNCTION MMUL1(A,B)
    IMPLICIT NONE
    REAL(c_double), INTENT(IN) :: A(:,:),B(:,:)
    REAL(c_double) :: MMUL1(SIZE(A,1),SIZE(B,2))
    INTEGER :: u
    u=ASSERT_EQ(SIZE(A,2),SIZE(B,1),"mmul1")
    CALL GEMM(A,B,MMUL1)    
  END FUNCTION MMUL1

  pure FUNCTION MMUL2(A,B)
    IMPLICIT NONE
    REAL(c_double), INTENT(IN) :: A(:,:),B(:)
    REAL(c_double) :: MMUL2(SIZE(A,1))
    INTEGER :: u
    u=ASSERT_EQ(SIZE(A,2),SIZE(B),"mmul2")
    CALL GEMV(A,B,MMUL2)    
  END FUNCTION MMUL2

  pure FUNCTION MMUL3(A,B)
    IMPLICIT NONE
    REAL(c_double), INTENT(IN) :: A(:),B(:,:)
    REAL(c_double) :: MMUL3(SIZE(B,2))
    INTEGER :: u
    u=ASSERT_EQ(SIZE(A),SIZE(B,1),"mmul3")
    CALL GEMV(TRANSPOSE(B),A,MMUL3)    
  END FUNCTION MMUL3

  pure FUNCTION MMUL4(A,B)
    IMPLICIT NONE
    REAL(c_double), INTENT(IN) :: A(:),B(:)
    REAL(c_double) :: MMUL4
    INTEGER :: u
  ...
  pure FUNCTION assert_eq2(n1,n2,string)
    CHARACTER(LEN=*), INTENT(IN) :: string
    ...
    if (n1 == n2) then
      ...
      !       string
       error STOP 'program terminated by assert_eq2'//trim(string)
    end if
  END FUNCTION assert_eq2

  pure FUNCTION assert_eq3(n1,n2,n3,string)
    CHARACTER(LEN=*), INTENT(IN) :: string
    ...
    if (n1 == n2 .and. n2 == n3) then
      ...
      !  write (*,*) 'nrerror: an assert_eq failed with this tag:', &
      !       string
       error STOP 'program terminated by assert_eq3'//trim(string)
    end if
  END FUNCTION assert_eq3

  pure FUNCTION assert_eq4(n1,n2,n3,n4,string)
    CHARACTER(LEN=*), INTENT(IN) :: string
    ...
    if (n1 == n2 .and. n2 == n3 .and. n3 == n4) then
      ...
      !  write (*,*) 'nrerror: an assert_eq failed with this tag:', &
      !       string
       error STOP 'program terminated by assert_eq4'//trim(string)
    end if
  END FUNCTION assert_eq4

  pure FUNCTION assert_eqn(nn,string)
    CHARACTER(LEN=*), INTENT(IN) :: string
    ...
    if (all(nn(2:) == nn(1))) then
      ...
      !  write (*,*) 'nrerror: an assert_eq failed with this tag:', &
      !       string
       error STOP 'program terminated by assert_eqn'//trim(string)
    end if
