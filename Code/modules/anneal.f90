...
CONTAINS
  ...
  SUBROUTINE Simulated_Annealing(temp0,tfactor,nsteps,x,func,max_jiter,id,k1)
   !> Performs the Simulated Annealing optimization algorithm to find the minimum of a function.
   !! This subroutine uses the Simulated Annealing algorithm to search for the minimum of a given function.
   !! The algorithm starts with an initial temperature and iteratively adjusts the temperature and explores the search space to find the minimum value.
   !! The function to be minimized is provided through the `FUNC` interface.
   !!
   !! @param temp0 The initial temperature for the Simulated Annealing algorithm.
   !! @param tfactor The temperature reduction factor applied at each iteration.
   !! @param nsteps The number of steps to take at each temperature level.
   !! @param x The initial guess for the minimum value. On output, it contains the final estimated minimum.
   !! @param func The function to be minimized. It should accept an array `x` as input and return a real value.
   !! @param max_jiter The maximum number of iterations for the Simulated Annealing algorithm.
   !! @param id An optional integer parameter to control the output verbosity. If set to 0, additional output will be printed.
   !! @param k1 An optional array of integers used to set the random number generator seed. If not provided, the seed will be generated based on the system clock.
   !!
   !! @return None
   IMPLICIT NONE
