MODULE INTEGRATION
  USE NRUTIL, ONLY : nrerror,arth,iminloc,NR_PI,assert_eq
  IMPLICIT NONE
  
  REAL(8) :: INT_EPS=1.0d-10
...
CONTAINS
  ...
  SUBROUTINE Gauss_Hermite(x,w)
    ...
    DO its=1,MAXIT
       ...
       WHERE (unfinished)
          ...
          z1=z
          z=z1-p1/pp
          unfinished=(ABS(z-z1) > 1.0d-12)! INT_EPS)
       END WHERE
  ...
  SUBROUTINE Lobatto_Gauss_Legendre(z,w,z1,z2)
    ...
    xold=2.0d0
    its=0
    DO WHILE (MAXVAL(SQRT((x-xold)**2))>INT_EPS)
       xold=x
       P(:,1)=1.0d0
       P(:,2)=x
       DO k=2,n
