! Module to generate sobol sequences, numbers and other stuff

! SOBOL, a FORTRAN90 code which computes elements of the Sobol
! quasirandom sequence, by Bennett Fox.

! A quasirandom or low discrepancy sequence, such as the Faure,
! Halton, Hammersley, Niederreiter or Sobol sequences, is "less random"
! than a pseudorandom number sequence, but more useful for such tasks as
! approximation of integrals in higher dimensions, and in global optimization.
...
! convergence.
!
! SOBOL is an adaptation of the INSOBL and GOSOBL routines in ACM TOMS Algorithm 647
! and ACM TOMS Algorithm 659. The original code can only compute the "next" element
! of the sequence. The revised code allows the user to specify the index of any
! desired element.
!
! At the request of a user, the library now contains two versions of the algorithm,
! one using the default integer and real precision, and one using the higher precision
! obtainable by requesting "(KIND=8)". The two sets of code are distinguished by names
! that begin with I4_ and I8_. I am aware that using the KIND attribute with an explicit
! value is not the correct way of asking for higher precision, but it's natural, and
! the correct way looks a little too awkward and obscure to me today. I am aware that
! many variables in the higher precision code need not themselves be calculated in
! higher precision. I'd just like to see if and how the higher precision code runs at all.
! !
! A remark by Joe and Kuo shows how to extend the algorithm from the original
! maximum spatial dimension of 40 up to a maximum spatial dimension of 1111.
! These changes have been implemented in the C++ and FORTRAN90 versions of the program.
! !
! The original, true, correct versions of ACM TOMS Algorithm 647 and
! ACM TOMS Algorithm 659 are available in the TOMS subdirectory of the NETLIB web site.
! The version displayed here has been converted to FORTRAN90, and other internal
! changes have been made to suit me.

module sobol
  implicit none
  public :: r8i8_uniform_01
  private
