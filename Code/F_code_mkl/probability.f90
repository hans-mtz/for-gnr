...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! PDF's !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  REAL(8) FUNCTION PDF_MULTINOMIAL(x,p,n)
    ! Returns the log of the pdf of a multinomial distribution with n trials and
    ! probabilities given by p evaluated at x
    ! Is the probability that in n trials there will be exactly
    ! x(i) occurrences of event i, whose probability is p(i)
    ! When N or SIZE(X) gets large the probability of a given event tends to get small
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: x(:),n
    REAL(8), INTENT(IN) :: p(:)
    PDF_MULTINOMIAL=EXP(LN_PDF_MULTINOMIAL(x,p,n))
  END FUNCTION PDF_MULTINOMIAL
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! LOG OF PDFS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  REAL(8) FUNCTION LN_PDF_MULTINOMIAL(x,p,n)
    ! Returns the log of the pdf of a multinomial distribution with n trials and
    ! probabilities given by p evaluated at x
    ! Is the probability that in n trials there will be exactly
    ! x(i) occurrences of event i, whose probability is p(i)
    ...
    INTEGER :: i
    REAL(8) :: pdf
    pdf=LOG_GAMMA(DBLE(n))
    DO i=1,SIZE(x)
       pdf=pdf + x(i)*LOG(p(i)) - LOG_GAMMA(DBLE(x(i)+1))
    END DO
    LN_PDF_MULTINOMIAL=pdf
  END FUNCTION LN_PDF_MULTINOMIAL
