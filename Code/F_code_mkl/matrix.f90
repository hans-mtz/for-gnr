MODULE MATRIX
  ...
  USE LAPACK95, ONLY : GESDD,GELS,SYTRF,SYTRI,POTRF,GETRF,GETRI
  USE BLAS95, ONLY : GER
  IMPLICIT NONE
  ...
  INTERFACE OLS
     MODULE PROCEDURE OLS_1,OLS_2,OLS_JIN
  END INTERFACE
...
!  END FUNCTION Outer_Product_r
  ...
  END FUNCTION Determinant
   ...
   FUNCTION OLS_1(Xin,Yin)
      IMPLICIT NONE
      REAL(8), INTENT(IN) :: Xin(:,:),Yin(:)
      REAL(8) :: X(SIZE(Xin,1),SIZE(Xin,2)+1),Y(SIZE(Yin),1)
      REAL(8) :: OLS_1(SIZE(Xin,2)+1)
      INTEGER :: info
      ...
      ELSE
            WRITE(*,*) "Error: Unable to solve the linear system."
            OLS_1 = 0.0d0
      END IF
   END FUNCTION OLS_1
